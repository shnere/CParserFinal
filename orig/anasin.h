	...
	while (1) {
      ...
      //printCharList(&ast);
			...
			if(actual.valor == 2 || actual.valor == 3){
				...
				// Reducciones al nombre de la variable
			...
			if(actual.valor == 6){
				...
				fprintf(stdout, "inputReal[i-1]: %s\n", inputReal[i-1]);
				localidad = getTokenIndex(inputReal[i-1]);
				//fprintf(stdout, "LOCALIDAD: %d\n", localidad);
				
				// Agregar nombre a el tokens[localidad]
				strcpy((char *) tokens[localidad].nombre, inputReal[i-1]);
			}
